...
/**
 * Updates one variation, writing only the fields it was given.
 */
class UpdateProductVariationAbility extends AbstractAbility {
    use RestDelegation;
    ...
    public function getName(): string {
        return self::qualify( 'update-product-variation' );
    }
    ...
    public function getLabel(): string {
        return __( 'Update a product variation', 'wp-mcp-abilities' );
    }
    ...
    public function getDescription(): string {
        return __(
            'Updates a WooCommerce product variation: attribute values, price, stock, reference or dimensions. Sending "attributes" replaces the current pairs, as the WooCommerce admin does.',
            'wp-mcp-abilities'
        );
    }
    ...
    public function getInputSchema(): array {
        $schema = $this->routeSchema( \WC_REST_Product_Variations_Controller::class, \WP_REST_Server::EDITABLE );
        ...
        $schema['properties'] = [
            ...
            'variation_id' => [
                'type'        => 'integer',
                'description' => __( 'Id of the variation to update.', 'wp-mcp-abilities' ),
            ],
        ] + ( $schema['properties'] ?? [] );
    ...
    public function execute( mixed $input = null ): array|\WP_Error {
        ...
        $body = $input;
        unset( $body['product_id'], $body['variation_id'] );
